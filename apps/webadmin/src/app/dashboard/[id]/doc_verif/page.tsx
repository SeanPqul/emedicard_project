                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className={`text-sm ${textColor}`}>
                          ...
                          <p className={warningLevel === 'severe' ? 'text-red-700' : warningLevel === 'warning' ? 'text-orange-700' : 'text-blue-700'}>
                            {rejectedCount > 3 
                              ? `More than 3 medical documents referred (${rejectedCount}/${totalDocs}). Applicant will be notified to see referred doctors.`
                              : rejectedCount === 1
                ...
                <div key={item._id} className="group border border-gray-200 rounded-2xl p-5 transition-all hover:shadow-md hover:border-teal-200 bg-white">
                  ...
                  {openReferralIndex === idx && (
                    <div className="mt-4 pt-4 border-t border-gray-200 bg-blue-50 -m-4 p-4 rounded-b-xl">
                      ...
                      <div className="space-y-4">
                        {/* Doctor Name Field - Only for medical documents */}
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                setDoctorName(name);
                                // Auto-update the details message if it was prefilled
                                setSpecificIssues(prev => {
                                  ...
                                  // If prev already starts with 'Failed medical requirement', overwrite it; else keep user's custom text
                                  if (!prev || prev.startsWith('Failed medical requirement')) return base;
                                  return prev;
                                });
                              }}
                              className="mt-1 block w-full px-3 py-2 text-gray-700 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                              placeholder="Enter doctor's name (e.g., Dr. Juan Dela Cruz)"
                            />
                          </div>
                        )}
                      ...
                      <div className="flex justify-end gap-3 mt-4">
                        ...
                        <button onClick={async () => {
                          try {
                            // Validate doctor name for medical documents
                            if (isMedicalDocument(item.fieldIdentifier)) {
                              if (!doctorName.trim()) {
                                throw new Error('Please enter doctor\'s name for medical document referral.');
                              }
                            }
                            ...
                            if (isMedicalDocument(item.fieldIdentifier)) {
                              ...
                              const currentStatus = item.status === 'Approved' || item.status === 'Rejected' ? item.status : 'Pending';
                              await reviewDocument({
                                documentUploadId: item.uploadId!,
                                status: currentStatus as 'Approved' | 'Rejected' | 'Pending',
