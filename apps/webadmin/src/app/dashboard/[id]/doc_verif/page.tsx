...
export default function DocumentVerificationPage({ params: paramsPromise }: PageProps) {
  ...
  // Fetch referral history for all documents (for showing referral details)
  const referralHistoryData = useQuery(api.documents.referralQueries.getReferralHistory, {
    applicationId: params.id
  });
  
  // Helper to get referral details for a document
  const getReferralDetails = (documentTypeId: Id<"documentTypes">) => {
    if (!referralHistoryData) return null;
    // Find the most recent referral for this document
    return referralHistoryData.find(ref => ref.documentTypeId === documentTypeId);
  };
  
  // Helper to get referral finding for a specific document (for pre-filling lab form)
  const getReferralFinding = (documentTypeId: Id<"documentTypes">): string | undefined => {
    // Check pending actions first (before notification sent)
    const item = data?.checklist.find(doc => doc.documentTypeId === documentTypeId);
    if (item?.uploadId) {
      const pending = getPendingAction(item.uploadId);
      if (pending?.finding) return pending.finding;
    }
    // After notification sent, query from documentReferralHistory table
    const referralInfo = getReferralDetails(documentTypeId);
    if (referralInfo?.findingDescription) {
