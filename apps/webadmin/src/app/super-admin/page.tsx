...
export default function SuperAdminPage() {
  ...
  const jobCategories = useQuery(api.jobCategories.getManaged.get); // Fetch all job categories
  ...
  const handleFilterThisMonth = () => {
    ...
    setStartDate(startOfMonth.getTime());
    setEndDate(endOfMonth.getTime());
  };
  ...
  const statusIconAndColorClasses: Record<string, { icon: React.ReactNode; colorClass: string }> = {
    "Total Applications": { icon: <StatIcon d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />, colorClass: "bg-blue-500" },
    "Submitted": { icon: <StatIcon d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />, colorClass: "bg-yellow-500" },
    "For Document Verification": { icon: <StatIcon d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />, colorClass: "bg-cyan-500" },
    "For Payment Validation": { icon: <StatIcon d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H4a3 3 0 00-3 3v8a3 3 0 003 3z" />, colorClass: "bg-purple-600" },
    "For Orientation": { icon: <StatIcon d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />, colorClass: "bg-indigo-500" },
    "Approved": { icon: <StatIcon d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />, colorClass: "bg-green-500" },
    "Rejected": { icon: <StatIcon d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />, colorClass: "bg-red-500" },
  ...
  // The middleware should handle redirection for non-super-admins, but this is a client-side fallback
  if (!adminPrivileges.isAdmin || adminPrivileges.managedCategories !== "all") {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <ErrorMessage title="Access Denied" message="You do not have Super Admin privileges to view this page." onCloseAction={() => router.push('/dashboard')} />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto py-8 px-6">
        <header className="mb-8">
          ...
          <p className="text-gray-600 mt-1">Overview and management of all health card applications.</p>
        </header>

        {/* Filter and Admin Actions */}
        <div className="bg-white p-4 rounded-xl shadow-sm mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex gap-4 flex-col sm:flex-row items-end"> {/* Align items to end for date pickers */}
              {/* Quick Filter Buttons */}
              <button onClick={handleFilterToday} className="bg-blue-600 text-white hover:bg-blue-700 px-4 py-2 rounded-lg font-medium transition-colors">
                Today
              </button>
              <button onClick={handleFilterThisMonth} className="bg-blue-600 text-white hover:bg-blue-700 px-4 py-2 rounded-lg font-medium transition-colors">
                This Month
              </button>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setStartDate(e.target.value ? new Date(e.target.value).setHours(0, 0, 0, 0) : undefined)}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEndDate(e.target.value ? new Date(e.target.value).setHours(23, 59, 59, 999) : undefined)}
                />
              </div>
              <button onClick={handleClearFilter} className="bg-gray-200 text-gray-800 hover:bg-gray-300 px-4 py-2 rounded-lg font-medium transition-colors">
                Clear Filter
              </button>
            </div>
            {/* Admin Actions */}
            <div className="flex-shrink-0">
              <button onClick={() => setIsModalOpen(true)} className="bg-emerald-600 text-white hover:bg-emerald-700 px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
          <StatCard 
            ...
            colorClass={statusIconAndColorClasses["Total Applications"].colorClass} 
          />
          {Object.entries(applicationsByStatus).map(([status, count]) => {
            const { icon, colorClass } = statusIconAndColorClasses[status] || { 
              icon: <StatIcon d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />, // Default icon (info circle)
              colorClass: "bg-gray-500" // Default color
            };
            return (
              <StatCard 
                key={status} 
