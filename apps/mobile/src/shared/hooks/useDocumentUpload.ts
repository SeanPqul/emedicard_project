...
import {
  ...
  smartCacheCleanup,
  type CachedDocument,
} from '@shared/services/storage/documentCache';
...
export const useDocumentUpload = (applicationId: Id<"applications">) => {
  ...
  const batchUploadCachedDocuments = useCallback(async (): Promise<{
    ...
    const { checkMemoryLimits } = await import('@shared/services/storage/documentCache');
  ...
  const bulkUploadWithQueue = useCallback(async (
    ...
    const { 
      ...
      checkMemoryLimits 
    } = await import('@shared/services/storage/documentCache');
